...
import dataset
import unet_models
from unet_models import UNet, Loss, FocalLoss
import utils
...
class StreetDataset(Dataset):
    def __init__(self, root: Path, size: Size, out_size=None, limit=None):
        self.image_paths = sorted(list(root.joinpath('images').glob('*.jpg')) + list(root.joinpath('images').glob('*.JPG')) + list(root.joinpath('images').glob('*.jpeg')))
...
class PredictionDataset:
    def __init__(self, root: Path, size: Size):
        self.paths = list(sorted(list(root.joinpath('images').glob('*.jpg')) + list(root.joinpath('images').glob('*.JPG')) + list(root.joinpath('images').glob('*.jpeg'))))
        self.size = size
